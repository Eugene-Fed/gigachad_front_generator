...
playwright install  # инициализируйте playwright
sudo apt-get install libevent-2.1-7t64 libavif16  # при необходимости установите необходимые зависимости playwright
```
...
### Скачайте корневой SSL сертификат МинЦифры

Скачайте [корневой сертификат](https://gu-st.ru/content/Other/doc/russian_trusted_root_ca.cer) МинЦифры и сохраните его в удобное для вас место.

Устанавливать их в систему не обязательно, OpenAI всё равно их не признает и будет выдавать ошибку, достаточно просто сохранить файл.
...
Пока что не поддерживается при запуске через OpenAI.

#### openai

Подключиться к ИИ через библиотеку OpenAI.

Пример:
```sh
python gigachat.py --openai
```

#### root_cert

Путь к корневому SSL-сертификату МинЦифры.

Пример:
```sh
python gigachat.py --root_cert /usr/local/share/ca-certificates/russian-trusted/russian_trusted_root_ca_pem.crt
```
